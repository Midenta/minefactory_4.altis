...
switch (_code) do {
    ...
    case 4: {
        if (playerSide == civilian && player getVariable["restrained",false]) then  {
            ...
            if (dialog) exitWith {};
            [] call husky_fnc_OpenEconomy;
        };
    };
	*/
	
	//Farmingsystem öffnen mit "NUM_0"
    case 82: {
    ...
    case 57: {
		if (!_shift && husky_barrier_active) then {
	...
	*/
	...
	case 79: {
		...
		if ((_ctrlKey) && (vehicle player == player)) then {
			cutText [format["Briefing"], "PLAIN DOWN"];
			player playMove "HubBriefing_loop";
		};
	};

	//Acts_AidlPsitMstpSsurWnonDnon_loop	//Gefesselt am Boden
	//Acts_AidlPsitMstpSsurWnonDnon02	//Gefesselt am Boden
	//Acts_AidlPsitMstpSsurWnonDnon_out	//Gefesselt am Boden

	//Acts_EpicSlit		//Spagant start
	//Acts_EpicSlit_in		//Spagant start vorbereitung
	//Acts_EpicSlit_out		//Spagant start spagat

	//Acts_ExecutionVictim_Loop		//Gefesselt auf den Knien

	//acts_miller_knockout	//Niederschlagen mit primär waffe

	//Unterhalten(STRG + Num 2)
	case 80: {
		if (_ctrlKey) then {_handled = true;};
		if ((_ctrlKey) && (vehicle player == player)) then {
			cutText [format["Unterhalten"], "PLAIN DOWN"];
			player playMove "Acts_CivilTalking_1";
		};
	};
	...
	case 81: {
		if (_ctrlKey) then {_handled = true;};
		if ((_ctrlKey) && (vehicle player == player)) then {
			cutText [format["Dehnung"], "PLAIN DOWN"];
			player playMove "Acts_JetsCrewaidRCrouchThumbup_loop";
		};
	};

	//Arm drehen(STRG + Num 4)
	case 75: {
		if (_ctrlKey) then {_handled = true;};
		if ((_ctrlKey) && (vehicle player == player)) then {
			cutText [format["Arm drehen"], "PLAIN DOWN"];
			player playMove "Acts_JetsMarshallingEnginesOn_loop";
		};
	};
			...
			if (!isnull cursorTarget) then {
				if (alive cursorTarget) then {
					if (cursorTarget isKindOf "Man") then {
						[husky_idcard_data,getplayeruid player] remoteExec ["husky_fnc_show_idcard",cursorTarget];
						hint format[localize "STR_capps_cur_error_success",name cursorTarget];
					} else {
						[husky_idcard_data,getplayeruid player] spawn husky_fnc_show_idcard;
		...
		if (!(playerSide isEqualto CIVILIAN) && cursorObject distance player < 3) then {
			if (!(husky_idcard_data isEqualto [])) then {
				if (!isnull cursorTarget) then {
					if (alive cursorTarget) then {
						if (cursorTarget isKindOf "Man") then {
							[husky_idcard_data,getplayeruid player,playerSide,(call husky_coplevel) + (call husky_mediclevel)] remoteExec ["husky_fnc_show_idcard",cursorTarget];
							hint format[localize "STR_capps_cur_error_success_card",name cursorTarget];
						} else {
							[husky_idcard_data,getplayeruid player,playerSide,(call husky_coplevel) + (call husky_mediclevel)] spawn husky_fnc_show_idcard;
							hint parseText "<t underline='true' size='1.8' color='#ff0000'>Warnung</t><br /><t size='1.5' color='#ffffff'>Du zeigst auf keine Person...</t>";
						};
					} else {
						[husky_idcard_data,getplayeruid player,playerSide,(call husky_coplevel) + (call husky_mediclevel)] spawn husky_fnc_show_idcard;
        ...
        if (_shift && (playerSide in ([west,independent,civilian])) && !isNull cursorObject && cursorObject isKindOf "Man" && isPlayer cursorObject && alive cursorObject && cursorObject distance player < 4 && speed cursorObject < 1) then {
            if ((animationState cursorObject) != "Incapacitated" && (currentWeapon player == primaryWeapon player || currentWeapon player == handgunWeapon player) && currentWeapon player != "" && !husky_knockout && !(player getVariable ["restrained",false]) && !husky_istazed && !husky_isknocked) then {
				...
				if ("ItemRadio" in assignedItems cursorTarget) then {
					...
					_defenceplace = "ItemRadio" createVehicle (player modelToWorld [0,0,0]);
				} else {
...
/*
	//9-Key
	case 10: {
		if (playerSide in [civilian] && (vehicle player != player) && {((driver vehicle player) == player)}) then {
	...
	*/
	...
	/*
	//CopSirenAN
		husky_actions = husky_actions + [player addAction["<t color='#001EFF'>Sirene Aktivieren</t>",husky_fnc_siren,"",0,false,false,'(typeOf vehicle player) in ["B_Heli_Light_01_F","B_Heli_Transport_01_F"]))']];
		//CopSirenAUS
		husky_actions = husky_actions + [player addAction["<t color='#001EFF'>Sirene Deaktivieren</t>",husky_fnc_siren,"",0,false,false,'(typeOf vehicle player) in ["B_Heli_Light_01_F","B_Heli_Transport_01_F"]))']];
	*/
	...
	//ALT+ A AUFBAUEN COP STATION
	/*case 30: {
		if(_alt && playerSide == west) then	{
			if("B_T_Truck_01_mover_F" == typeOf (vehicle player) && player == driver (vehicle player)) then {
				if((speed _vehicle) > 0) exitWith {hint "Fahrzeug muss stillstehen bevor es sich aufbaut !!!"};

				[vehicle player] remoteExec ["TON_fnc_mobileCopStation", RSERV];
	...
	*/
	...
	/*
	Falls jemand wieder den Stringtable nehmen will:
	STR_MISC_soundnormal
	STR_MISC_soundfade
	*/
	...
	case  	63: {
		if (_shift) then {
			...
			//[] spawn husky_fnc_ClanHorn;
			["Gang-Tag aktuallisiert", "Information"] spawn husky_fnc_createNotification;
	...
	*/
	...
	case  	62: {
		if (_shift) then {
			...
			["Gang-Tag NICHT aktuallisiert", "Kacke", true, [1, 1, 1, 1], [1,0,0,1]] spawn husky_fnc_createNotification;
			//Hintsilent "Gang-Tag aktuallisiert!"
		};
	};*/
	...
	case 38: {
        ...
        if (!_alt && !_ctrlKey) then { [] call husky_fnc_ar; };
	};*/

    //F Key
   /* case 33: {
        if ((playerSide isEqualTo INDEPENDENT) && !((vehicle player) isEqualTo player) && !husky_siren_active && ((driver (vehicle player)) isEqualTo player)) then {
            [] spawn {
                husky_siren_active = true;
                sleep 4.7;
                husky_siren_active = false;
        ...
        if ((playerSide in [WEST,CIVILIAN]) && !((vehicle player) isEqualTo player) && !husky_siren_active && ((driver (vehicle player)) isEqualTo player)) then {
            if (((vehicle player) getVariable ["playerSide_player",sideUnknown]) isEqualTo WEST) then {
                        [] spawn {
                            husky_siren_active = true;
                            sleep 4.7;
                            husky_siren_active = false;
                        ...
                        if (_veh getVariable ["siren",false]) then {
                            ...
                            titleText [localize "STR_MISC_SirensON","PLAIN"];
                            _veh setVariable ["siren",true,true];
                            [_veh] remoteExec ["husky_fnc_copSiren",RCLIENT];
                        };
                } else {
                    titleText ["Die Sirene kann nur in Polizeifahrzeugen verwendet werden.","PLAIN"];
                };
          };
    };*/
    ...
    /*case 22: {
        if (!_alt && !_ctrlKey) then {
            ...
            if (_veh isKindOf "House_F" && {playerSide in [civilian]}) then {
                if (_veh in husky_vehicles && player distance _veh < 14) then {
                    _door = [_veh] call husky_fnc_nearestDoor;
                ...
                _locked = locked _veh;
                if (_veh in husky_vehicles && player distance _veh < 12) then {
                    if (_locked isEqualTo 2) then {
                        if (local _veh) then {
                            _veh lock 0;
                        } else {
                        ...
                        [_veh,"UnlockCarSound"] remoteExec ["husky_fnc_say3D",RANY];
						hintSilent parseText "<img size='1' color='#FFFFFF' image='icons\unlock.paa'/><t size='1'> Du hast dein Fahrzeug aufgeschlossen.</t>";
                    } else {
                        if (local _veh) then {
							_veh lock 2;
                        } else {
                        ...
                        [_veh,"LockCarSound"] remoteExec ["husky_fnc_say3D",RANY];
						hintSilent parseText "<img size='1' color='#FFFFFF' image='icons\lock.paa'/><t size='1'> Du hast dein Fahrzeug abgesperrt.</t>";
                    };
                };
            };
        };
