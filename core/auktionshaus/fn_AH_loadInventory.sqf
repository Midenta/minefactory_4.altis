	...
	} else {
{
	...
	switch (_type) do {
		case "imenu": {
			_itemDisplayName = ([_item] call husky_fnc_fetchCfgDetails) select 1;
			...
			if (_price == 0) then {_priceDisplay = ""} else {_priceDisplay = format["- Preis $%1",[_price]call husky_fnc_numberText];};
			};
		case "ymenu": {
...
/* while {true} do
{
	...
	if (isNull (findDisplay 15600)) exitWith {};
	sleep 1;
	((findDisplay 15600) displayCtrl 15660) ctrlSetText "textures\Menus\auktionshaus\auktionshaus.jpg";
	if (isNull (findDisplay 15600)) exitWith {};
	sleep 0.1;
	((findDisplay 15600) displayCtrl 15660) ctrlSetText "textures\Menus\auktionshaus\auktionshaus.jpg";
	if (isNull (findDisplay 15600)) exitWith {};
