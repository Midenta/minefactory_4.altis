...
[] spawn
{
	...
	while {true} do
	{
		...
		if(!([west,getPos player,30] call husky_fnc_nearUnits) && (player getVariable["tied", false]) && isNull objectParent player) exitWith {
			...
			detach player;
			hintSilent "Du hast es geschafft, dich zu befreien."
		};
	};
};

hint format ["%1 hat dich gefesselt.",name _civ];
