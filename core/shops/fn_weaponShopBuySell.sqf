...
if ((uiNamespace getVariable ["Weapon_Shop_Filter",0]) isEqualTo 1) then {
    ...
    hint parseText format [localize "STR_Shop_Weapon_Sold",_itemInfo select 1,[_price] call husky_fnc_numberText];
    ...
    if (!isNil "_hideout" && {!isNil {group player getVariable "gang_bank"}} && {(group player getVariable "gang_bank") >= _price}) then {
        ...
        if (_action) then {
			...
			[getPlayerUID player, "Buy-I-Items GANG", format ["%1 - %2 hat eine %3 für %4 € gekauft. Bargeld: %5 € Bankkonto: %6 €",profileName,(getPlayerUID player),_itemInfo select 1,[_price] call husky_fnc_numberText,[c126] call husky_fnc_numberText,[b126] call husky_fnc_numberText]] remoteExec ["HC_fnc_AdvancedLog",HC_Husky];
            ...
            if (_price > c126) exitWith {hint localize "STR_NOTF_NotEnoughMoney"};
            hint parseText format [localize "STR_Shop_Weapon_BoughtItem",_itemInfo select 1,[_price] call husky_fnc_numberText];
			[getPlayerUID player, "Buy-I-Items 2", format ["%1 - %2 hat eine %3 für %4 € gekauft. Bargeld: %5 € Bankkonto: %6 €",profileName,(getPlayerUID player),_itemInfo select 1,[_price] call husky_fnc_numberText,[c126] call husky_fnc_numberText,[b126] call husky_fnc_numberText]] remoteExec ["HC_fnc_AdvancedLog",HC_Husky];
            c126 = c126 - _price;
            [_item,true] call husky_fnc_handleItem;
        };
    } else {
        if (_price > c126) exitWith {hint localize "STR_NOTF_NotEnoughMoney"};
        hint parseText format [localize "STR_Shop_Weapon_BoughtItem",_itemInfo select 1,[_price] call husky_fnc_numberText];
		[getPlayerUID player, "Buy-I-Items", format ["%1 - %2 hat eine %3 für %4 € gekauft. Bargeld: %5 € Bankkonto: %6 €",profileName,(getPlayerUID player),_itemInfo select 1,[_price] call husky_fnc_numberText,[c126] call husky_fnc_numberText,[b126] call husky_fnc_numberText]] remoteExec ["HC_fnc_AdvancedLog",HC_Husky];
        c126 = c126 - _price;
        [_item,true] call husky_fnc_handleItem;
    };
};
