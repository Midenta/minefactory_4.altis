...
if ([false,_type,_amount] call husky_fnc_handleInv) then {
	...
	if(((_grp getVariable ["gang_id",-1]) isEqualTo -1)) then {
		[getPlayerUID player, "Sell-V-Items", format ["%1 - %2 hat %3 von %4 für %5 € verkauft. Preis Item: %6 € Bargeld: %7 € Bankkonto: %8 €",profileName,(getPlayerUID player),_amount,(localize _name),[_price] call husky_fnc_numberText,[_price2] call husky_fnc_numberText,[c126] call husky_fnc_numberText,[b126] call husky_fnc_numberText]] remoteExec ["HC_fnc_AdvancedLog",HC_Husky];
    };
	if(!((_grp getVariable ["gang_id",-1]) isEqualTo -1)) then {
		...
		if((_tax isEquaLTo 0) && {"PAYTAX" in _playerpermission}) then {
			[getPlayerUID player, "Sell-V-Items GANG", format ["%1 - %2 hat %3 von %4 für %5 € verkauft. Preis Item: %6 € Bargeld: %7 € Bankkonto: %8 €",profileName,(getPlayerUID player),_amount,(localize _name),[_price] call husky_fnc_numberText,[_price2] call husky_fnc_numberText,[c126] call husky_fnc_numberText,[b126] call husky_fnc_numberText]] remoteExec ["HC_fnc_AdvancedLog",HC_Husky];
		};
		if(!(_tax isEquaLTo 0) && {"PAYTAX" in _playerpermission}) then {
			...
			[getPlayerUID player, "Sell-V-Items GANG Steuer", format ["%1 - %2 hat %3 von %4 für %5 € verkauft. Spieler: %6 € Gang Steuer %7 € Preis Item: %8 € Bargeld: %9 € Bankkonto: %10 €",profileName,(getPlayerUID player),_amount,(localize _name),[_price1] call husky_fnc_numberText,[_price] call husky_fnc_numberText,[_val] call husky_fnc_numberText,[_price2] call husky_fnc_numberText,[c126] call husky_fnc_numberText,[b126] call husky_fnc_numberText]] remoteExec ["HC_fnc_AdvancedLog",HC_Husky];
		};
	};
