...
if ((_price * _amount) > b126 && {!isNil "_hideout" && {!isNil {group player getVariable "gang_bank"}} && {(group player getVariable "gang_bank") <= _price * _amount}}) exitWith {hint localize "STR_NOTF_NotEnoughMoneyEC"};
...
if ([true,_type,_amount] call husky_fnc_handleInv) then {
    if (!isNil "_hideout" && {!isNil {group player getVariable "gang_bank"}} && {(group player getVariable "gang_bank") >= _price}) then {
        ...
        if (_action) then {
            ...
            if ((_price * _amount) > b126) exitWith {[false,_type,_amount] call husky_fnc_handleInv; hint localize "STR_NOTF_NotEnoughMoneyEC";};
            hint format [localize "STR_Shop_Virt_BoughtItem",_amount,(localize _name),[(_price * _amount)] call husky_fnc_numberText];
            b126 = b126 - _price * _amount;
        };
    } else {
        if ((_price * _amount) > b126) exitWith {hint localize "STR_NOTF_NotEnoughMoneyEC"; [false,_type,_amount] call husky_fnc_handleInv;};
        hint format [localize "STR_Shop_Virt_BoughtItem",_amount,(localize _name),[(_price * _amount)] call husky_fnc_numberText];
        b126 = b126 - _price * _amount;
    };
...
if ([true,_type,_amount] call husky_fnc_handleInv) then {
    if (!isNil "_hideout" && {!isNil {group player getVariable "gang_bank"}} && {(group player getVariable "gang_bank") >= _price}) then {
        ...
        if (_action) then {
            hint format[localize "STR_Shop_Virt_BoughtGang",_amount,(localize _name),[(_price * _amount)] call husky_fnc_numberText];
			[getPlayerUID player, "Buy-V-Items GANG EC", format ["%1 - %2 hat %3 von %4 für %5 € gekauft. Bargeld: %6 € Bankkonto: %7 €",profileName,(getPlayerUID player),_amount,(localize _name),[(_price * _amount)] call husky_fnc_numberText,[c126] call husky_fnc_numberText,[b126] call husky_fnc_numberText]] remoteExec ["HC_fnc_AdvancedLog",HC_Husky];
            _funds = group player getVariable "gang_bank";
            _funds = _funds - (_price * _amount);
            group player setVariable["gang_bank",_funds,true];
            ...
            if ((_price * _amount) > b126) exitWith {[false,_type,_amount] call husky_fnc_handleInv;};
            hint format[localize "STR_Shop_Virt_BoughtItemEC",_amount,(localize _name),[(_price * _amount)] call husky_fnc_numberText];
			[getPlayerUID player, "Buy-V-Items 2 EC", format ["%1 - %2 hat %3 von %4 für %5 € gekauft. Bargeld: %6 € Bankkonto: %7 €",profileName,(getPlayerUID player),_amount,(localize _name),[(_price * _amount)] call husky_fnc_numberText,[c126] call husky_fnc_numberText,[b126] call husky_fnc_numberText]] remoteExec ["HC_fnc_AdvancedLog",HC_Husky];
            b126 = b126 - _price * _amount;
        };
    } else {
        if ((_price * _amount) > b126) exitWith { [false,_type,_amount] call husky_fnc_handleInv;};
        hint format[localize "STR_Shop_Virt_BoughtItemEC",_amount,(localize _name),[(_price * _amount)] call husky_fnc_numberText];
		[getPlayerUID player, "Buy-V-Items EC", format ["%1 - %2 hat %3 von %4 für %5 € gekauft. Bargeld: %6 € Bankkonto: %7 €",profileName,(getPlayerUID player),_amount,(localize _name),[(_price * _amount)] call husky_fnc_numberText,[c126] call husky_fnc_numberText,[b126] call husky_fnc_numberText]] remoteExec ["HC_fnc_AdvancedLog",HC_Husky];
        b126 = b126 - _price * _amount;
    };
};
