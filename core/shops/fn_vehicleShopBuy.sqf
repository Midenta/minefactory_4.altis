...
if ((c126 < _purchasePrice) and (_mode)) exitWith {hint format [localize "STR_Shop_Veh_NotEnough",[_purchasePrice - c126] call husky_fnc_numberText];closeDialog 0;};
if (c126 < _purchasePrice) exitWith {hint format [localize "STR_Shop_Veh_NotEnoughRent",[_purchasePrice - c126] call husky_fnc_numberText];closeDialog 0;};
...
c126 = c126 - _purchasePrice;
[0] call SOCK_fnc_updatePartial;
if (_mode) then {
    hint format [localize "STR_Shop_Veh_Bought",getText(configFile >> "CfgVehicles" >> _className >> "displayName"),[_purchasePrice] call husky_fnc_numberText];
} else {
    hint format [localize "STR_Shop_Veh_Rent",getText(configFile >> "CfgVehicles" >> _className >> "displayName"),[_purchasePrice] call husky_fnc_numberText];
};
...
if (_mode) then {
	if !(_className in husky_SETTINGS(getArray,"vehicleShop_rentalOnly")) then {
		if (husky_HC_isActive) then {
			[(getPlayerUID player),playerSide,_vehicle,_colorIndex,_plate] remoteExecCall ["HC_fnc_vehicleCreate",HC_husky];
		} else {
			[(getPlayerUID player),playerSide,_vehicle,_colorIndex,_plate] remoteExecCall ["TON_fnc_vehicleCreate",RSERV];
		};
	};
};
*/
[getPlayerUID player, "boughtVehicle", format ["%1 - %2 hat sich das Fahrzeug %3 für %4 € gekauft. Bargeld: %5 € Bankkonto: %6 €",profileName,(getPlayerUID player),_className,[_purchasePrice] call husky_fnc_numberText,[c126] call husky_fnc_numberText,[b126] call husky_fnc_numberText]] remoteExec ["HC_fnc_AdvancedLog",HC_Husky];
