...
husky_call_start = compileFinal '
	...
	lbClear _playerslb;
	if ((count husky_med_calls) > 0) then {
		{
...
husky_call_lbchange = compileFinal '
	...
	if (((count husky_med_calls)-1) < _cur) exitwith {
	...
	_playerdata = husky_med_calls select _cur;
...
husky_call_accept = compileFinal '
	...
	if (((count husky_med_calls)-1) < _cur) exitwith {hint localize "STR_DC_reload"};
	_temp = husky_med_calls select _cur;
	...
	husky_med_calls set[_cur,_temp];
...
husky_call_delete = compileFinal '
	...
	if ((_playerslb lbData _cur) isEqualTo "-1") exitWith {};
	if (((count husky_med_calls)-1) < _cur) exitwith {hint localize "STR_DC_reload"};
	_data = husky_med_calls select _cur;
	...
	if (!_result) exitWith {};
	husky_med_calls set [_cur,-1];
	husky_med_calls = husky_med_calls - [-1];
	publicVariable "husky_med_calls";
	_playerslb lbDelete _cur;
	if ((lbSize _playerslb) isEqualTo 0) then {
		_playerslb lbAdd (localize "STR_DC_nodispatches");
...
husky_call_showcall = compileFinal '
	...
	if (((count husky_med_calls)-1) < _lbcur) exitwith {hint localize "STR_DC_reload"};
	_lbcur = husky_med_calls select _lbcur;
...
husky_call_sendmessage = compileFinal '
	...
	if (((count husky_med_calls)-1) < _cur) exitwith {hint localize "STR_DC_reload"};
	_to = (husky_med_calls select _cur) select 5;
