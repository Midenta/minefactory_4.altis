...
if (visibleMap) then {
...

    {
        if !(_x isEqualTo player) then {
            _markerss = createMarkerLocal [format ["%1_marker",_x],visiblePosition _x];
            _markerss setMarkerColorLocal "ColorIndependent";
            _markerss setMarkerTypeLocal "Select";
            _markerss setMarkerTextLocal format ["%1", _x getVariable ["realname",name _x]];
    ...
    //Loop through and create markers.
    {
        ...
        _markers pushBack _marker;
			...
			_markerss = createMarkerLocal [format ["%1_marker",player],visiblePosition player];
            _markerss setMarkerColorLocal "ColorIndependent";
            _markerss setMarkerTypeLocal "Select";
            _markerss setMarkerTextLocal format ["%1", player getVariable ["realname",name player]];

            _markersMedecin pushBack [_markerss,player];
    while {visibleMap} do {
        {
            private ["_unit"];
            _unit = _x select 1;
