...
if ((lbCurSel 1505) == -1) exitWith {hint "Du hast nichts ausgewählt"; closeDialog 0;};
_index = -1;
_state = -1;
_control = ((findDisplay 8000) displayCtrl 1505);
...
_index = getNumber(missionconfigfile >> "CfgTuning" >> str(playerSide) >> "smokescreen" >> "index");
_state = getNumber(missionconfigfile >> "CfgTuning" >> str(playerSide) >> "smokescreen" >> _dataArr >> "state");
