...
if!((_unit getVariable ["realname",name _unit]) in husky_Bekannte) then {
husky_Bekannte pushBack (_unit getVariable ["realname",name _unit]);
...
_language = if(language in ["German"]) then {language} else {getText(missionConfigFile >> "aapps_idcard_config" >> "cfgPrimaryLanguage")};
