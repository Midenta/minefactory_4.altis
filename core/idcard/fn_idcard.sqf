...
if(_type isEqualTo 0) exitWith {//ONLOAD
	...
	if((!isNil "husky_idcard_fake") && {husky_idcard_fake}) then {
		husky_idcard_fake = false;
		_access_btn ctrlSetEventHandler
		[
			"ButtonDown",
			"
				[1,true] spawn husky_fnc_idcard
			"
		];
	} else {
		_access_btn ctrlSetEventHandler
...
if(_type isEqualTo 1) exitWith {
	...
	if(_fakeid) then {
		...
		_result = [format[localize "STR_capps_gui_question",[_money] call husky_fnc_numberText], localize "STR_capps_gui_title", localize "STR_capps_gui_accept", localize "STR_capps_gui_decline"] call BIS_fnc_guiMessage;
		if(!_result) exitWith {};
		if(c126 < _money) exitWith {
			hint format[localize "STR_capps_not_enough",[_money] call husky_fnc_numberText];
		};
		c126 = c126 - _money;
