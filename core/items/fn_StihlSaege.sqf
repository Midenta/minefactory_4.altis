...
*/
private["_building","_door","_doors","_cpRate","_title","_progressBar","_titleText","_cp","_ui","_unit"];

_building = param [0,ObjNull,[ObjNull]];

if(isNull _building) exitWith {};
if (isNil "husky_sthil_uses") then {husky_sthil_uses = 0;};
...
_fnc_Gefangnis = {
	...
	_unit = player;
	[_unit,"TS500I",150,1] remoteExecCall ["husky_fnc_say3D",0];
	...
	husky_sthil_uses = husky_sthil_uses + 1;
	husky_action_inUse = false;
	if(husky_sthil_uses >= 5) then {
		[false,"stihlsaege",1] call husky_fnc_handleInv;
		husky_sthil_uses = 0;
	};
	...
	[getPlayerUID player, "STIHL Säge", format ["%1 - %2 hat eine STIHL Säge benutzt. %3 ",profileName,(getPlayerUID player),_building]] remoteExec ["HC_fnc_AdvancedLog",HC_Husky];

};


if(typeOF(_building) in ["Land_Net_Fence_Gate_F","Land_BackAlley_01_l_gate_F","Land_ConcreteWall_01_l_gate_F","Land_ConcreteWall_01_m_gate_F","Land_Medevac_house_V1_F","Land_Research_house_V1_F"])then {[] spawn _fnc_Gefangnis};
