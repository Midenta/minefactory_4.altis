...
if (!([false,"fuelFull",1] call husky_fnc_handleInv)) exitWith {};
...
if (!alive player) exitWith {};
if (husky_interrupted) exitWith {husky_interrupted = false; titleText[localize "STR_NOTF_ActionCancel","PLAIN"];};


switch (true) do {
    case (_vehicle isKindOF "LandVehicle"): {
        if (!local _vehicle) then {
            [_vehicle,(Fuel _vehicle) + 0.5] remoteExecCall ["husky_fnc_setFuel",_vehicle];
        } else {
    ...
    case (_vehicle isKindOf "Air"): {
        if (!local _vehicle) then {
            [_vehicle,(Fuel _vehicle) + 0.2] remoteExecCall ["husky_fnc_setFuel",_vehicle];
        } else {
    ...
    case (_vehicle isKindOf "Ship"): {
        if (!local _vehicle) then {
            [_vehicle,(Fuel _vehicle) + 0.35] remoteExecCall ["husky_fnc_setFuel",_vehicle];
        } else {
...
[true,"fuelEmpty",1] call husky_fnc_handleInv;