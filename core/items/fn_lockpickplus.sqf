...
if (!([false,"lockpickplus",1] call husky_fnc_handleInv)) exitWith {husky_action_inUse = false;};
...
if (!_isVehicle) then {
    ...
    if (_dice < 100) then {
        ...
        [getPlayerUID player, "LockpickPlus erfolgreich", format ["%1 - %2 hat Lockpick an %3 erfolgreich genutz. Bankkonto: %3 €  Bargeld: %4 € ",profileName,(getPlayerUID player),_curTarget,[b126] call husky_fnc_numberText,[c126] call husky_fnc_numberText]] remoteExec ["HC_fnc_AdvancedLog",HC_Husky];
