...
_safeInfo = _safe getVariable ["airdopinv",-1];
_safeInfoitem = _safe getVariable ["airdopinvitem",""];
if (_safeInfo < 1) exitWith {closeDialog 0; hint localize "STR_Civ_VaultEmpty";};

_str = M_CONFIG(getText,"VirtualItems",_safeInfoitem,"displayName");
_shrt = M_CONFIG(getText,"VirtualItems",_safeInfoitem,"variable");
_tInv lbAdd format ["[%1] - %2",_safeInfo,(localize _str)];
_tInv lbSetData [(lbSize _tInv)-1,_shrt];
_icon = M_CONFIG(getText,"VirtualItems",_safeInfoitem,"icon");
