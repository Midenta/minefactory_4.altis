...
if (player distance _house > 13) exitWith {husky_action_inUse = false; titleText[localize "STR_House_Raid_TooFar","PLAIN"]};
...
_value = 0;
{
    ...
    if (ITEM_ILLEGAL(_var) isEqualTo 1) then {
        if (!(ITEM_SELLPRICE(_var) isEqualTo -1)) then {
            ...
            _houseInvVal = _houseInvVal - (([_var] call husky_fnc_itemWeight) * _val);
...
if (_value > 0) then {
    [0,"STR_House_Raid_Successful",true,[[_value] call husky_fnc_numberText]] remoteExecCall ["husky_fnc_broadcast",RCLIENT];
    b126 = b126 + round(_value / 2);
    [1] call SOCK_fnc_updatePartial;

    _house setVariable ["Trunk",[_houseInvData,_houseInvVal],true];

    if (husky_HC_isActive) then {
        [_house] remoteExecCall ["HC_fnc_updateHouseTrunk",HC_husky];
    } else {
