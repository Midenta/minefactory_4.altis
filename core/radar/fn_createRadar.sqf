	...
	if (([getPosASL _radar,getDir _radar,120,getPosASl _veh] call BIS_fnc_inAngleSector)&& _speed > _limit) then {
		if (driver _veh isEqualTo player) then { 
			...
			if (de100_luciandjuli_bc < _ticket) then {
				...
				if (_ticket+(call husky_fnc_calcDebts) > de100_luciandjuli_bc) then {
					_ticket = de100_luciandjuli_bc-(call husky_fnc_calcDebts) max 0;
				};
				de100_luciandjuli_bc = de100_luciandjuli_bc-_ticket;
			};
			de100_mSpace setVariable ["cV",de100_luciandjuli_mula+de100_luciandjuli_bc];

			[format["Sie wurden geblitzt!<br/>Gemessene Geschwindigkeit: %1km/h<br/>Erlaubt: %2km/h<br/><br/>%3€ wurden von deinem Bankkonto abgebucht", ceil _speed, _limit,[_ticket] call husky_fnc_numberText],false,"slow"] call husky_fnc_notification_system;
		};
		0 spawn {
