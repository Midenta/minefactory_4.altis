...
private _type = "";
private _typeName = "";
husky_net_dropped = true;
titleText[localize "STR_NOTF_NetDrop","PLAIN"];
sleep 5;
if (_fish isEqualTo []) exitWith {titleText[localize "STR_NOTF_NetDropFail","PLAIN"]; husky_net_dropped = false;};
{
    if (_x isKindOf "Fish_Base_F") then {
        ...
        if ([true,_type,1] call husky_fnc_handleInv) then {
            deleteVehicle _x;
