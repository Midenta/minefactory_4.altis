...
if (isClass (missionConfigFile >> "ProcessAction" >> _type)) then {
    ...
    _conditions = M_CONFIG(getText,"ProcessAction",_type,"conditions");
} else {_filter = true;};
...
if ([_conditions] call husky_fnc_levelCheck) then {
    _hasLicense = true;
};
...
if (_hasLicense) then {
    ...
    if (c126 < _cost) exitWith {hint format [localize "STR_Process_License",[_cost] call husky_fnc_numberText]; "progressBar" cutText ["","PLAIN"]; husky_is_processing = false; husky_action_inUse = false;};
    ...
    "progressBar" cutText ["","PLAIN"];
    if (_minimumConversions isEqualTo (_totalConversions call BIS_fnc_lowestNum)) then {hint parseText "<t underline='true' size='1.8' color='#7cff00'>Informant</t><br /><t size='1.5' color='#ffffff'>Du hast deine Materialien verarbeitet!</t>";} else {hint parseText "<t underline='true' size='1.8' color='#ff0000'>Warnung</t><br /><t size='1.5' color='#ffffff'>Nur ein Teil der Materialien konnte verarbeitet werden, sonst würde das maximale Gewicht erreicht werden.</t>";};
    c126 = c126 - _cost;
