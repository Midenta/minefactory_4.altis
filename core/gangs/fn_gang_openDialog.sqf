...
if (!(husky_gangData isEqualTo [-1,-1]) OR _bool) then {
    _group = group player;
    if (!_bool) then {
        _ownid = husky_gangData select 0;
        if (!(_ownid isequalTo -1)) then {
            ...
            if (!(_gangid isEqualTo _ownid)) exitWith {
                ...
                [_ownid] spawn husky_fnc_gang_findGang;
            };
        };
    };
    createDialog "husky_gang_overview";
    ...
    if (_bool && {!(_permission isEqualTo 0)}) then {
        ...
        _listbox lbSetData [_val,"0 call husky_fnc_gang_reassign"];
    ...
    if ((_permission isEqualTo 0) OR {"TAG" in _playerpermission} OR {"TAX" in _playerpermission}) then {
        ...
        _listbox lbSetData [_val,"0 call husky_fnc_gang_settings"];
        //_listbox lbSetPicture [_val,""];
        _val = _val + 1;
    };
    ...
    if (getNumber(missionConfigFile >> "gang_config" >> "temporary_gangs") isEqualTo 1) then {
        createDialog "husky_gang_buymenu";
    } else {
        createDialog "husky_gang_buymenu_small";
    };
    ctrlSetText[220070,([getNumber(missionConfigFile >> "gang_config" >> "temp_gang_price")] call husky_fnc_numberText) + "€"];
    ctrlSetText[220071,([getNumber(missionConfigFile >> "gang_config" >> "perm_gang_price")] call husky_fnc_numberText) + "€"];
};
