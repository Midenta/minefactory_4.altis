 ...
 *
 */
if ([220080,"dialog_gang_changegroup"] call husky_fnc_gang_activedialog) exitWith {hint localize "STR_aapps_gang_dialog_inuse"};
disableSerialization;
createDialog "husky_gang_groupchange";
...
_group = group player;
husky_gang_members =+ (_group getVariable ["gang_members",[]]);
//_index = 0;
{
...
_combo ctrlSetEventhandler ["LBSelChanged", '
    (_this select 1) call husky_fnc_gang_loadgroups;
...
_listbox1 ctrlSetEventHandler ["LBDblClick",'
    ...
    _entryarr set [1,1];
    husky_gang_members set [_entry,_entryarr];
    if (lbData[220082,0] isEqualTo "[]") then {lbClear 220082};
    _groupname = 1 call husky_fnc_fetch_groupname;
...
_listbox2 ctrlSetEventHandler ["LBDblClick",'
    ...
    _groupname = _rank call husky_fnc_fetch_groupname;
    if (_groupname isEqualTo "") then {_groupname = "Deleted"};
