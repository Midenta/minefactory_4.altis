...
if (_display getVariable ["shiftingActive",false] && !(_display getVariable ["lockPickBroke",false]) && !(_display getVariable ["shaking",false])) then {
	private _tolerance = _display getVariable ["tolerance",getNumber(missionConfigFile >> "husky_Settings" >> "lpick_sys_tolerance")];
	private _perc = _display getVariable ["perc",getNumber(missionConfigFile >> "husky_Settings" >> "lpick_sys_perc")];
