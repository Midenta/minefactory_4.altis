...
if (husky_HC_isActive) then {
	[_uid,lbCurSel _dropBox,_display getVariable ["type",0]] remoteExec ["HC_fnc_updateRank",HC_husky];
} else {
	[_uid,lbCurSel _dropBox,_display getVariable ["type",0]] remoteExec ["TON_fnc_updateRank",2];
};


[_listBox,lbCurSel _listBox] call husky_fnc_staffManagerLbChange;