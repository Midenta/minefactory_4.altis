 ...
 *  
 */
husky_call_task = taskNull;
husky_call_random = 0;
husky_call_start = compileFinal '
	...
	lbClear _playerslb;
	if ((count husky_cop_calls) > 0) then {
		{
...
';
husky_call_lbchange = compileFinal '
	disableSerialization;
	_display = findDisplay 1011001;
	_playerslb = _display displayCtrl 1011002;
	...
	if (((count husky_cop_calls)-1) < _cur) exitwith {
	...
	_playerdata = husky_cop_calls select _cur;
	...
	_bool = false;
	if (isNull husky_call_task) then {
		_bool = true;
	} else {
		if (!(((taskDescription husky_call_task) select 0) isEqualTo (_playerData select 2))) then {
			_bool = true;
		};
	};
	if (_bool) then {
		ctrlSetText[1011012,"a3\3den\data\cfgwrapperui\cursors\3denplacewaypointmulti_ca.paa"];
		buttonSetAction[1011012,"0 call husky_call_addWaypoint"];
	} else {
	...
	husky_call_random = floor random 10000;
	_tempvar = husky_call_random;
	while {husky_call_random isEqualTo _tempvar} do {
...
';
husky_call_accept = compileFinal '
	disableSerialization;
	_display = findDisplay 1011001;
	_playerslb = _display displayCtrl 1011002;
	_bearb = _display displayCtrl 1011009;
	...
	if (((count husky_cop_calls)-1) < _cur) exitwith {hint localize "STR_DC_reload"};
	_temp = husky_cop_calls select _cur;
	...
	husky_cop_calls set[_cur,_temp];
	publicVariable "husky_cop_calls";
	_bearb ctrlSetText (name player);
	_playerslb lbSetPicture [_cur, "icons\ico_set.paa"];
	ctrlEnable[1011013,false];
';
husky_call_delete = compileFinal '
	disableSerialization;
	_display = findDisplay 1011001;
	_playerslb = _display displayCtrl 1011002;
	_time = _display displayCtrl 1011007;
	...
	if ((_playerslb lbData _cur) isEqualTo "-1") exitWith {};
	if (((count husky_cop_calls)-1) < _cur) exitwith {hint localize "STR_DC_reload"};
	_data = husky_cop_calls select _cur;
	...
	if (!_result) exitWith {};
	husky_cop_calls set [_cur,-1];
	husky_cop_calls = husky_cop_calls - [-1];
	publicVariable "husky_cop_calls";
	_playerslb lbDelete _cur;
	if ((lbSize _playerslb) isEqualTo 0) then {
		_playerslb lbAdd (localize "STR_DC_nodispatches");
...
';
husky_call_msgchange = compileFinal '
...
';
husky_call_showcall = compileFinal '
	...
	if (((count husky_cop_calls)-1) < _lbcur) exitwith {hint localize "STR_DC_reload"};
	_lbcur = husky_cop_calls select _lbcur;
...
';
husky_call_sendmessage = compileFinal '
	private "_msg";
	...
	if (((count husky_cop_calls)-1) < _cur) exitwith {hint localize "STR_DC_reload"};
	_to = (husky_cop_calls select _cur) select 5;
	//[_msg,name player,4] remoteExec ["TON_fnc_clientMessage",_to];
	[_msg,profilename,0] remoteExec ["TON_fnc_clientMessage",_to];	//Standard
	hint localize "STR_DC_sendmsg";
';
husky_call_addWaypoint = compileFinal '
	_lbcur = lbcursel 1011002;
	if (_lbcur isEqualTo -1) exitWith {hint localize "STR_DC_notselected"};
	if (lbdata[1011002,_lbcur] isEqualTo "-1") exitWith {};
	if (((count husky_cop_calls)-1) < _lbcur) exitwith {hint localize "STR_DC_reload"};
	...
	if (!(isNull husky_call_task)) then {
		player removeSimpleTask husky_call_task;
	};
	husky_call_task = player createSimpleTask [format[(localize "STR_DC_title") + ": %1",_lbcur select 0]];
	husky_call_task setSimpleTaskDestination _pos;
	husky_call_task setSimpleTaskDescription [_lbcur select 2,format[(localize "STR_DC_title") + ": %1",_lbcur select 0],""];
	husky_call_task setTaskState "ASSIGNED";
	player setCurrentTask husky_call_task;
	husky_call_task spawn {
		while {true} do {
			if (isNull husky_call_task) exitWith {};
			if ((player distance (taskDestination husky_call_task)) < 30) exitWith {
				player removeSimpleTask husky_call_task;
			};
			_bool = false;
			_text = taskDescription husky_call_task;
