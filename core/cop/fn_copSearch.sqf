...
if (count _invs > 0) then {
    ...
    if (_illegal > 0) then {
        ...
        if (husky_HC_isActive) then {
            [getPlayerUID _civ,_civ getVariable ["realname",name _civ],"58"] remoteExecCall ["HC_fnc_wantedAdd",HC_husky];
        } else {
            [getPlayerUID _civ,_civ getVariable ["realname",name _civ],"58"] remoteExecCall ["husky_fnc_wantedAdd",RSERV];
        };

    };

    if (husky_HC_isActive) then {
        [getPlayerUID _civ,_civ getVariable ["realname",name _civ],"57"] remoteExecCall ["HC_fnc_wantedAdd",HC_husky];
    } else {
        [getPlayerUID _civ,_civ getVariable ["realname",name _civ],"57"] remoteExecCall ["husky_fnc_wantedAdd",RSERV];
    };

    [0,"STR_Cop_Contraband",true,[(_civ getVariable ["realname",name _civ]),[_illegal] call husky_fnc_numberText]] remoteExecCall ["husky_fnc_broadcast",west];
} else {
...
if (_robber) then {
    [0,"STR_Cop_Robber",true,[(_civ getVariable ["realname",name _civ])]] remoteExecCall ["husky_fnc_broadcast",RCLIENT];
};
