...
private _level = husky_currentExpLevel;
private _levelConfigBase = _baseConfig >> "Levels";
private _currentExpLevelConfig = _levelConfigBase >> format["Level_%1", husky_currentExpLevel];
...
private _iteratedExp = husky_currentExp;
...
while {_dif > 0} do {
    ...
    if !(_level isEqualTo husky_currentExpLevel) then {
...
husky_currentExp = husky_currentExp + _expGained;
husky_currentPerkPoints = husky_currentPerkPoints + _gainedPerkPoints;
