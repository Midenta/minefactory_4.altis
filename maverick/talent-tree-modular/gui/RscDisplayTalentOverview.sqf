...
try {
	...
	// -- Initialized?
	if !(missionNamespace getVariable ["husky_perksInitialized", false]) exitWith {};
	...
	_updateGUI = {
		private _currentLevelConfig = missionConfigFile >> "Maverick_TTM" >> "Levels" >> format["Level_%1", husky_currentExpLevel];
		private _nextLevelConfig = missionConfigFile >> "Maverick_TTM" >> "Levels" >> format["Level_%1", husky_currentExpLevel + 1];
		...
		private _currentLevelRelativeExp = (husky_currentExp - _currentLevelBottomExp);
		private _currentLevelProgress = if (_currentLevelRelativeExp != 0) then {((husky_currentExp - _currentLevelBottomExp) / (_currentLevelTopExp - _currentLevelBottomExp))} else {0};

		// -- Display data
		_ctrlStrucTextCurrentRank ctrlSetStructuredText parseText format ["<t size='3' shadow='0' align='center'>%1</t>", husky_currentExpLevel];
		_ctrlTextRankName ctrlSetText getText (_currentLevelConfig >> "displayName");

		if (isClass _nextLevelConfig) then {
			_ctrlTextStatusInfo ctrlSetText format ["XP: %1/%2, Perk Points Available: %3", husky_currentExp - _currentLevelBottomExp, _currentLevelTopExp - _currentLevelBottomExp, husky_currentPerkPoints];
			_ctrlProgress progressSetPosition _currentLevelProgress;
		} else {
			_ctrlTextStatusInfo ctrlSetText format ["XP: %1, Maximum Level, Perk Points Available: %2", husky_currentExp - _currentLevelBottomExp, husky_currentPerkPoints];
	...
	switch (_mode) do
	{
		...
		{
			// -- Not loaded in yet?
			if (!husky_perksInitialized) exitWith {
			...
			[husky_currentExpPerks, []] call _generateTreeView;
		...
		case "onTreeSelectionChanged":
		{
			...
			private _hasRequiredLevel = ((getNumber (_selectedPerkConfig >> "requiredLevel")) <= husky_currentExpLevel);
			private _limitedToFactions = getArray (_selectedPerkConfig >> "limitToSides");
			private _hasParents = if (_requiredPerk != "") then {[husky_currentExpPerks, getText (_selectedPerkConfig >> "requiredPerk")] call mav_ttm_fnc_hasPerk;} else {true};

			husky_currentExpPerks call _iterate;

			if (((count _limitedToFactions) == 0) || {(str playerSide) in _limitedToFactions}) then {
				if (_hasRequiredLevel) then {
					if (_hasParents) then {
						if (!_ownsPerk) then {
							if ((getNumber (_selectedPerkConfig >> "requiredPerkPoints")) > husky_currentPerkPoints) then {
		...
		case "onButtonPurchaseClick":
		{
			...
			if ((getNumber (_selectedPerkConfig >> "requiredPerkPoints")) > husky_currentPerkPoints) exitWith {
			...
			husky_currentPerkPoints = husky_currentPerkPoints - (getNumber (_selectedPerkConfig >> "requiredPerkPoints"));
