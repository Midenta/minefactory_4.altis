...
#define c126 (missionNamespace getVariable getText (MAVCFG >> "cashVar"))
...
switch _mode do {
    case "onLoad": {
		...
		_ctrlCashOnHand ctrlSetText format [getText (MAVCFG >> "localization" >> "msgCashOnHand"), SYMBOL, [c126] call BIS_fnc_numberText];
    ...
    case "onTreeChanged": {
		...
		private _afford = (MAVV(Total) + _price) > c126;
	...
	case "complete": {
		...
		missionNamespace setVariable [getText (MAVCFG >> "cashVar"), (c126 - _cash)];
