...
if (_masterConfig != "") then {
	// Log
	//diag_log format ["Tuning: Anwendung der folgenden Daten auf das Fahrzeug %1", _data];
	...
	if ((_data select 1) == "") then {
		...
		// Log
		//diag_log format ["Tuning: Keine Farbe gefunden! Standard bis %1", (_data select 1)];
	};
