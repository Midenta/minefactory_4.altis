...
_veh = vehicle player;
_position = getMarkerPos "jp_tuning";	//mav_tuning_originalPosition findEmptyPosition [0,150,typeOf _veh];
...
// Save vehicle to DB
if (missionNamespace getVariable ["husky_HC_isActive", false]) then {
	[_veh getVariable "vehicleID", _veh getVariable ["mav_tuning",[]]] remoteExec ["mav_tuning_fnc_saveTuningToDB", HC_husky];
} else {
