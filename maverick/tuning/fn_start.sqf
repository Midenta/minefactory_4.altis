...
if (vehicle player == player) exitWith {hint "Du musst in einem Fahrzeug sein"};
if (driver vehicle player != player) exitWith {hint "Du bist nicht der Fahrer des Fahrzeugs!"};
if ((count crew vehicle player) > 1) exitWith {hint "Du musst alleine im Fahrzeug sein!"};
if (isNil {(vehicle player) getVariable "vehicleID"}) exitWith {hint "Das Fahrzeug muss zuerst in einer Garage gelagert werden, bevor es getunt werden kann!"};
if (diag_tickTime - mav_tuning_lastVisit < 10) exitWith {hint "Sie besuchen bereits den Tuning-Shop!"};
...
// Not tuneable?
if (_configName == "") exitWith {hint "Dieses Fahrzeug kann nicht getunt werden"};
...
if ((_data select 1) != "") then {
	...
	//diag_log "Tuning: Keine benutzerdefinierte Textur gefunden";
};
