...
fnc_RESET = {
	...
	if (_debug) then {
		_sidechatHint = format["Anzahl gleichzeitiger Kameras: %1",count _conCurrentCams];
	...
	if (count _conCurrentCams > 4) then {
		{
			...
			camDestroy _x;
			_monitor setObjectTextureglobal [0,"textures\images\banner.paa"];
		}foreach _conCurrentCams;
		
		_conCurrentCams=[];server setvariable ["CONCURRENTCAMS",_conCurrentCams,true];
						
		if (_debug) then {
			_sidechatHint = format["CAMS löschen: %1",count _conCurrentCams];
			PAPABEAR sideChat _sidechatHint;
			GlobalSideChat = _sidechatHint; 
			publicVariable "GlobalSideChat";
		};
...
fnc_PROCESS_LIVE = {
	...
	switch (_class) do {
		...
		case "B_huskyboat": {_cam attachto [(vehicle _unit),[0,0.15,-0.3], "neck"];};
		...
		case "O_huskyboat": {_cam attachto [(vehicle _unit),[0,0.15,-0.3], "neck"];};
	...
	if not(isnil "_x") then {
		if not(isNull _x) then {
			...
			if (name _x == "Error: No unit" ) then {
				_uniqueName=format ["<t color=""#ff0000"">" + ("ÜBERWACHUNG : %1") + "</t>",_x];
			...
			_monitor addAction [_uniqueName, "call fnc_PROCESS_LIVE",[_x,_debug],0,false,true,"","(side _this == west)"];		
			_uniqueName=format ["<t color=""#00ff00"">" + ("SATELLIT : %1") + "</t>",name _x];
...
if (_debug) then {
	_sidechatHint = format["Gesamtzahl der Kameras: %1", _cameraCount];
	PAPABEAR sideChat _sidechatHint;
	GlobalSideChat = _sidechatHint; 
	publicVariable "GlobalSideChat";
};
