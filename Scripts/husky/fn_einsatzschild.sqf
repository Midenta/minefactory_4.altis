...
if (husky_Einsatzschild) exitWith {hint "Du kannst keine zwei Einsatzschilder gleichzeitig benutzen !!"};
husky_Einsatzschild = true;
...
player addAction ["EINSATZSCHILD NEHMEN",
{
	...
	[] spawn
	{
		...
		while {husky_EinsatzschildInUse} do
		{
	...
	{  
		...
		{  
			hitted = 1;  
			hit = [];  
			_oldhusky = (getDammage _unit);  
			_dam = _dame - _oldhusky;  
		
			if (_dam > 0.2) then { _dam = 0.2; }
			else { _dam = _dam * 0.7; };  
			
			_dame = _oldhusky + _dam;    
		} else {  
...
}, "", 0, false, false, "", "(husky_Einsatzschild && !husky_EinsatzschildInUse && ((vehicle player) == player))"];
