...
if ((getMarkerPos ( format ["stig_sz_markerA%1",_this] )) isEqualTo [0,0,0]) exitWith {
	hint format ["Gefahrenzone %1 kann nicht gelöscht werden, da sie nicht erstellt wurde.",_this];
};
deleteMarker (format["stig_sz_markerA%1",_this]);
deleteMarker (format["stig_sz_markerB%1",_this]);
(uiNamespace getVariable "stig_sz_status") ctrlSetText "Gefahrenzone entfernt. Bereit.";
