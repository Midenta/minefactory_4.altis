...
if!((getMarkerPos ( format ["stig_sz_markerA%1",_this] )) isEqualTo [0,0,0]) exitWith { hint format ["Gefahrenzone %1 kann nicht erstellt werden. Du musst sie erst entfernen.",_this]; };
(uiNamespace getVariable "stig_sz_status") ctrlSetText "Wähle das Gefahrenzonenzentrum.";
_a = stig_sz_mapclick;
...
if!((getMarkerPos ( format ["stig_sz_markerA%1",_this] )) isEqualTo [0,0,0]) exitWith { hint "Fehler: Gefahrenzonenerstellung wurde automatisch abgebrochen. Grund: Mehrere Personen erstellten gleichzeitig die gleiche Gefahrenzone."; stig_sz_vorschaumarker_var setMarkerSizeLocal [0,0]; };
...
_m setMarkerText (format["Gefahrenzone %1",_this]);
...
(uiNamespace getVariable "stig_sz_status") ctrlSetText "Gefahrenzone erstellt. Bereit.";
