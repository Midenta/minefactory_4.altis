...
private _trunkData = cat_Lockergang_vehicle getVariable ["Trunk",[[],0]];
private _inv = _trunkData select 0;

private _weight = _amount * ([_item,_type] call cat_Lockergang_fnc_itemWeight);
private _index = [_item,_inv,-1,-1] call cat_Lockergang_fnc_index;
if (_add) then {
    ...
    cat_Lockergang_vehicle setVariable["Trunk",[_inv,(_trunkData select 1) + _weight],true];
} else {
