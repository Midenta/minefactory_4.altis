...
switch (_type) do {
	case 0 : {
		...
		switch(_storage) do {
			case 0 : {
				_return = floor ((((group player) getVariable "cat_Lockergang_maxWeight") - ((group player) getVariable "cat_Lockergang_carryWeight")) / _weight);
			};
			case 1 : {
				_return = floor ((husky_maxWeight - husky_carryWeight) / _weight);
			};
			case 2 : {
				_return = floor (((([cat_Lockergang_vehicle] call husky_fnc_vehicleWeight) select 0) - (([cat_Lockergang_vehicle] call husky_fnc_vehicleWeight) select 1))  / _weight);
			};
		};
	};
	case 1 : {
		switch(_storage) do {
			case 0 : {
				private _weight = [_item,_type] call cat_Lockergang_fnc_itemWeight;
                _return = floor ((((group player) getVariable "cat_Lockergang_maxWeight") - ((group player) getVariable "cat_Lockergang_carryWeight")) / _weight);
			};
			case 1 : {
