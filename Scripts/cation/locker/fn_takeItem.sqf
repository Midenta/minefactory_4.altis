...
if (_type isEqualTo 0) then {
    if (_viewMode isEqualTo 1) then {
        ...
        [getPlayerUID player, "Locker nehmen 1", format ["%1 - %2 hat %3 %4 %5 %6 %7 . Bargeld: %8 € Bankkonto: %9 € Coin: %10 ",profileName,getplayeruid player,_item,_type,_index,_amount,_ammoCount,[c126] call husky_fnc_numberText,[b126] call husky_fnc_numberText,[COINS] call husky_fnc_numberText]] remoteExec ["HC_fnc_AdvancedLog",HC_Husky];
    
    } else {
        if ([true,_item,_amount] call husky_fnc_handleInv) then {
            [_item,false,_type,_index,_amount,_ammoCount] call cat_locker_fnc_handleItemLocker;
            [getPlayerUID player, "Locker nehmen 2", format ["%1 - %2 hat %3 %4 %5 %6 %7 . Bargeld: %8 € Bankkonto: %9 € Coin: %10 ",profileName,getplayeruid player,_item,_type,_index,_amount,_ammoCount,[c126] call husky_fnc_numberText,[b126] call husky_fnc_numberText,[COINS] call husky_fnc_numberText]] remoteExec ["HC_fnc_AdvancedLog",HC_Husky];
    ...
    [_item,false,_type,_index,_amount,_ammoCount] call cat_locker_fnc_handleItemLocker;
    [getPlayerUID player, "Locker nehmen 3", format ["%1 - %2 hat %3 %4 %5 %6 %7 . Bargeld: %8 € Bankkonto: %9 € Coin: %10 ",profileName,getplayeruid player,_item,_type,_index,_amount,_ammoCount,[c126] call husky_fnc_numberText,[b126] call husky_fnc_numberText,[COINS] call husky_fnc_numberText]] remoteExec ["HC_fnc_AdvancedLog",HC_Husky];
