...
} forEach getArray(missionConfigFile >> "huskyCfgVehicles" >> _item >> "textures"); // for each skin of vehicle
...
[_vehicle,_colorIndex] call husky_fnc_colorVehicle; // set skin
[_vehicle] call husky_fnc_clearVehicleAmmo; // remove items inside vehicle
...
husky_vehicles pushBack _vehicle; // add to keyring
if (getNumber(missionConfigFile >> "Cation_CraftingV2" >> "HeadlessSupport") isEqualTo 0) then { // if headless client is disabled
    ...
    if (husky_HC_isActive) then { // if headless client is active
     	[getPlayerUID player,playerSide,_vehicle,1] remoteExecCall ["HC_fnc_keyManagement",HC_husky]; // call headless client keymanagement
        [(getPlayerUID player),playerSide,_vehicle,_colorIndex] remoteExecCall ["HC_fnc_vehicleCreate",HC_husky]; // call headless client to add vehicle to database
