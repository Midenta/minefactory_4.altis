/*
    ...
    Version: 1.1
    1.0 - Initial release
    1.1 - Bugfix: craft backpack to player inventory
*/
params [
	["_item","",[""]],
	["_vItem",false,[false]]
...
if (_vItem) then { // if vItem
	private _weight = [_item] call husky_fnc_itemWeight; // get item weight
	_return = floor ((husky_maxWeight - husky_carryWeight) / _weight); // calculate amount of item that can be stored in player inventory
} else { // ArmA item
