/*
	...
	1.2 - Bugfix: in some edge cases worng amount of items was seized
*/
params [
	["_itemsNeeded",[],[[]]],
	["_amount",1,[0]],
	...
	if (_location isEqualTo 0) then { // take it from prefered storage: player inventory
		{
			...
			if (_itemPlayer isEqualTo _itemNeeded) then { // if item is equal to needed item
				...
				if (_amount > 0) then { // if needed amount is greater than player amount
					...
					[false,_itemNeeded,_amountPlayer] call husky_fnc_handleInv; // remove item from player inventory
					...
					[false,_itemNeeded,_amountNeeded] call husky_fnc_handleInv; // remove item from player inventory
