/*
    ...
    Version: 1.4
    1.0 - Initial release
    1.1 - Bugfix: craft multiple items (seize right amount of items)
    1.2 - Enhancement: Move while crafting
    1.3 - Enhancement: More debugging info
    1.4 - Enhancement: Added option to specify experience points player receives when items was crafted successfully
*/
...
if ((getNumber(missionConfigFile >> "Cation_CraftingV2" >> "version")) > 4) then { // if version 4 or greater
    if ((player getVariable ["playerSurrender",false]) || husky_isknocked) exitWith { // check some variables
        if ((getNumber(missionConfigFile >> "Cation_CraftingV2" >> "DebugMode")) isEqualTo 1) then { // if debug mode is enabled
            //diag_log "cationstudio crafting: [ERROR] player is not allowed to craft. Function cat_craftingV2_fnc_processItems"; // log entry
        };
    };
...
if (!(_nickName isEqualTo "")) then { // nickname defined
    ...
    if (_vItem) then { // is vItem
        ...
        private _itemInfo = [_item] call husky_fnc_fetchCfgDetails; // get item details
...
if (husky_is_processing || husky_action_inUse) exitWith {}; // if action or processing is in use
husky_is_processing = true; // set processing variable to in use: true
husky_action_inUse = true; // set action in Use variable to in use: true
...
for "_a" from 1 to _amount step 1 do { // for each item that should be crafted
    ...
    for "_i" from 0 to 100 step 1 do { // from 0 to 100
        if (isNull findDisplay 5100) then { // dialog closed
            _dialogClosed = true;
            private _ui = uiNameSpace getVariable ["husky_progress", displayNull];
    ...
    try {
        if (_location isEqualTo 0) then { //save in player inventory
            if (_vItem) then { // if vItem
                if !([true,_item,1] call husky_fnc_handleInv) throw "not enough space"; // save item in player inventory else -> error
            } else { // else Arma item
...
husky_is_processing = false; // set processing variable to in use: false
husky_action_inUse = false; // set action in Use variable to in use: false
