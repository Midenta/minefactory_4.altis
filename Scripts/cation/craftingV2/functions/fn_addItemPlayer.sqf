...
if (isNull player || !alive player || (player getVariable ["restrained",false]) || (player getVariable ["Escorting",false]) || husky_istazed) exitWith {closeDialog 0;}; // player not allowed to use crafting
if ((getNumber(missionConfigFile >> "Cation_CraftingV2" >> "version")) > 4) then {
    if ((player getVariable ["playerSurrender",false]) || husky_isknocked) exitWith {closeDialog 0;}; // player not allowed to use crafting
};
...
_details = [_item] call husky_fnc_fetchCfgDetails; // fetch details
