/*
	...
	Attention: taking items from locker only works for vItems!
...
if (_return) then { // if no error
    ...
    if (getNumber(missionConfigFile >> "Cation_Locker" >> "HeadlessSupport") isEqualTo 0) then { // if headless client is not enabled
        ...
        if (husky_HC_isActive) then { // if headless client is active
            [cat_locker_trunk,cat_locker_level,getPlayerUID player,playerSide] remoteExecCall ["cat_locker_fnc_updateTrunkHC",HC_husky]; // send update method to headless client
