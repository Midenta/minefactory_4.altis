...
}forEach (missionNamespace getVariable ["Gambling_AllDatas",[]]);

_inPot = false;
if(!isNull player) then {
    _uid = getPlayerUID player;
    for "_i" from 0 to count(missionNamespace getVariable ["Gambling_AllDatas",[]])-1 step 1 do {
        if(((missionNamespace getVariable ["Gambling_AllDatas",[]]) select _i select 0) isEqualTo _uid) then {
            _userInput = ((missionNamespace getVariable ["Gambling_AllDatas",[]]) select _i select 1);
            _inPot = true;
        };
    };
    if(!_inPot) exitWith {
